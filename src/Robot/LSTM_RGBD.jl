function Robot_LSTM_RGBD(w,h,c,x)
    #RGB
...
function Robot_LSTM_RGBD_w()
    ...
    rgb_w = load(pretrainedPath("Robot_LSTM_RGB.jld"),"w")
    d_w = load(pretrainedPath("Robot_LSTM_D.jld"),"w")
    w[1:24] = rgb_w[1:24]
    w[25:48] = d_w[1:24]
    return w
end

function Robot_LSTM_RGBD_loss(w,h,c,x,y,t,D)
    total = 0.0
    for i in max(t-D,1):t
        ypred = Robot_LSTM_RGBD(w,h,c,KnetArray(x[:,:,:,i:i]))
