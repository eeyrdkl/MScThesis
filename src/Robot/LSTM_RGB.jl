function Robot_LSTM_RGB(w,h,c,x)
...
function Robot_LSTM_RGB_w()
    ...
    w[1:24] = load(pretrainedPath("Robot_Conv_RGB.jld"),"w")[1:24]
    return w
end

function Robot_LSTM_RGB_loss(w,h,c,x,y,t,D)
    total = 0.0
    for i in max(t-D,1):t
        ypred = Robot_LSTM_RGB(w,h,c,KnetArray(x[:,:,:,i:i]))
