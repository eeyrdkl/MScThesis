...
function DAVIS_LSTM_w()
    ...
    w[1:24] = load(pretrainedPath("DAVIS_Conv.jld"),"w")[1:24]
    return w
end

function DAVIS_LSTM_loss(w,h,c,x,y,t,D)
    total = 0.0
    for i in max(t-D,1):t
        ypred = DAVIS_LSTM(w,h,c,KnetArray(x[:,:,:,i:i]))
