...
function vKITTI_Conv_RGBD_w()
    ...
    rgb_w = load(pretrainedPath("vKITTI_Conv_RGB.jld"),"w")
    d_w = load(pretrainedPath("vKITTI_Conv_D.jld"),"w")
    w[1:24] = rgb_w[1:24]
    w[25:48] = d_w[1:24]
    return w
end

function vKITTI_Conv_RGBD_loss(w,x,y)
    pixelwiseSoftloss(Array(vKITTI_Conv_RGBD(w,x)),y)
end