function vKITTI_RNN_RGBD(w,h,x)
    #RGB
    ...
    #28,28,512,1
    x3 = x1.+x2
    h = relu(conv4(w[49],x3;padding=1) .+ conv4(w[50],h;padding=1) .+ w[51])

    x3 = relu(conv4(w[52],h).+w[53])
    x3 = deconv4(w[54],x3;padding=1,stride=6).+w[55]
end

function vKITTI_RNN_RGBD_w()
    w = Array[
        #RGB
        #Conv64
        ...
        xavier(Float32,3,3,512,512),     zeros(Float32,1,1,512,1),
        #RNN
        xavier(Float32,3,3,512,512),     xavier(Float32,3,3,512,512),   zeros(Float32,1,1,512,1), #W,U,bias
        #Scores
        xavier(Float32,1,1,512,13),      zeros(Float32,1,1,13,1),
        #Deconv
        bilinear(Float32,32,32,13,13),   zeros(Float32,1,1,13,1),
    ]
    rgb_w = load(pretrainedPath("vKITTI_RNN_RGB.jld"),"w")
    d_w = load(pretrainedPath("vKITTI_RNN_D.jld"),"w")
    w[1:24] = rgb_w[1:24]
    w[25:48] = d_w[1:24]
    return w
end

function vKITTI_RNN_RGBD_loss(w,h,x,y,t,D)
    total = 0.0
    for i in max(t-D,1):t
        ypred = vKITTI_RNN_RGBD(w,h,KnetArray(x[:,:,:,i:i]))
