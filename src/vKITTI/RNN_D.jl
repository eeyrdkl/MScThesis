function vKITTI_RNN_D(w,h,x)
    ...
    #28,28,512,1
    h = relu(conv4(w[25],x;padding=1) .+ conv4(w[26],h;padding=1) .+ w[27])
    
    x = relu(conv4(w[28],h).+w[29])
    x = deconv4(w[30],x;padding=1,stride=6).+w[31]
end

function vKITTI_RNN_D_w()
    w = Array[
        #Conv64
    ...
    w[1:24] = load(pretrainedPath("vKITTI_Conv_D.jld"),"w")[1:24]
    return w
end

function vKITTI_RNN_D_loss(w,h,x,y,t,D)
    total = 0.0
    for i in max(t-D,1):t
        ypred = vKITTI_RNN_D(w,h,KnetArray(x[:,:,:,i:i]))
